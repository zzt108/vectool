...
public sealed class TestRunnerHandler : FileHandlerBase
{
    ...
    /// </summary>
    /// <param name="ui">Optional UI for progress and messaging.</param>
    ...
    /// <summary>
    /// Runs 'dotnet test' using the provided solution path and writes the output into a timestamp-free, deterministic file
    /// named 'TestResults-{vectorStoreName}.{branchName}.txt' in the solution directory, then registers it in Recent Files.
    /// </summary>
    ...
    public async Task<string?> RunTestsAsync(string solutionPath, string vectorStoreName, List<string> selectedFolders)
    {
        ...
        try
        {
            ...
            // Register in Recent Files if available
