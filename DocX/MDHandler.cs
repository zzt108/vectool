...
namespace DocXHandler
{
    public class MDHandler
    {
        ...
        public static void ConvertFilesToMD(string folderPath, string outputPath)
        {
        ...
        public static void ConvertSelectedFoldersToMD(List<string> folderPaths, string outputPath)
        {
            // Create a new Word document
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(outputPath, DocumentFormat.OpenXml.WordprocessingDocumentType.Document))
            {
        ...
        public static void ExportSelectedFoldersToMarkdown(List<string> folderPaths, string outputPath)
        {
            using (StreamWriter writer = new StreamWriter(outputPath))
            {
                foreach (string folderPath in folderPaths)
                {
                    ProcessFolderForMarkdown(folderPath, writer);
                    // Process subfolders
                    string[] subfolders = Directory.GetDirectories(folderPath);
                    foreach (string subfolder in subfolders)
                    {
                        ProcessFolderForMarkdown(subfolder, writer);
                    }
                }
            }
        }

        private static void ProcessFolderForMarkdown(string folderPath, StreamWriter writer)
        {
            log.Debug(folderPath);
